      ...
      <div className="bg-white shadow-sm p-6 sm:p-8 mb-6 transition-all duration-300">
        <div className="flex flex-col sm:flex-row sm:items-center gap-8">
          ...
          <div className="relative shrink-0">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-300 rounded-full flex items-center justify-center transition-transform duration-200">
  ...
  />
        ...
        <div className="px-4 sm:px-4 py-4 pb-1 overflow-x-auto ">
          ...
          <div className="grid grid-cols-8 pr-0 px-4 py-4 sm:pb-9">
            ...
            <div className="font-semibold text-gray-700 text-sm lg:text-lg w-30">{t("profile.facebookName")}</div>
            <div className="font-semibold text-gray-700 text-sm lg:text-lg w-25">{t("profile.memberCode")}</div>
            <div className="font-semibold text-gray-700 text-sm lg:text-lg w-25">{t("profile.priorityDate")}</div>
            <div className="font-semibold text-gray-700 text-sm lg:text-lg pl-2">{t("profile.price")}</div>
            <div className="font-semibold text-gray-700 text-sm lg:text-lg pl-6">Status</div>
          </div>
          </div>
          </div>
          <div className="border-b border-[#ee6786ff]" />
      ...
      <div className="bg-white shadow-sm pt-1 px-3 sm:px-4 pb-6"> 
          <div className="space-y-4 mt-4">
            ...
            {!loading && displayedGroups.length === 0 && (
              <div className="text-center py-12 text-gray-500 transition-all duration-300">
                {activeTab === "orders" ? "No pending tickets found." : "No received tickets found."}
              </div>
            )}
                    ...
                    <div key={`${orderGroup.orderId}-${ticketIndex}`}>
                      ...
                      <div className="hidden md:grid grid-cols-8 gap-4 py-3 pl-2 min-w-[1040px]">
                        <div className="text-base lg:text-lg whitespace-nowrap">
                          {ticketIndex === 0 && <div className="font-medium text-black">{orderGroup.orderId || "00001"}</div>}
                        </div>
                        <div className="text-base lg:text-lg font-medium text-black truncate max-w-[14ch]">
                          {ticketIndex === 0 && <div className="truncate">{orderGroup.eventTitle || "Event"}</div>}
                        </div>
                        <div className="text-base lg:text-lg font-medium text-black truncate max-w-[16ch]">{ticket.userName || "—"}</div>
                        <div className="text-base lg:text-lg text-black truncate max-w-[18ch]">{ticket.facebookName || "—"}</div>
                        <div className="text-base lg:text-lg text-black whitespace-nowrap">{ticket.memberCode || "—"}</div>
                        <div className="text-base lg:text-lg font-medium text-black truncate whitespace-nowrap">{ticket.priorityDate || "—"}</div>
                        <div className="text-base lg:text-lg font-medium text-black truncate whitespace-nowrap">{price}</div>
                        <div className="text-base lg:text-lg pl-3">
                          <button
                            type="button"
                            onClick={() => openOrderDetails(orderGroup, ticket)}
                            className={`inline-block px-3 py-1 rounded border text-sm lg:text-lg font-medium cursor-pointer ${getStatusColor(status)} hover:scale-105 hover:opacity-90 transition-all duration-200`}
                      ...
                      <div className="md:hidden text-black mt-3 first:mt-0 rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-2 transition-all duration-200">
                        {ticketIndex === 0 && (
                          <>
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-medium text-black">{orderGroup.orderId || "00001"}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="font-semibold">{t("profile.event")}</span>
                              <span className="font-medium text-black max-w-[55%] text-right">{orderGroup.eventTitle || "Event"}</span>
                            </div>
                          </>
                        )}
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="text-black max-w-[55%] text-right">{ticket.facebookName || "—"}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between text-sm">
                          <span className="font-semibold">{t("profile.priorityDate")}</span>
                          <span className="text-black">{ticket.priorityDate || "—"}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between items-center pt-2">
                          ...
                          <button
                            type="button"
                            onClick={() => openOrderDetails(orderGroup, ticket)}
                            className={`px-3 py-1 rounded border text-sm font-medium ${getStatusColor(status)} hover:opacity-90 transition-colors duration-200`}
                            style={{ background: "transparent" }}
                          >
                            {status}
                          </button>
    ...
    {showChangePassword && (
      <div className="fixed inset-0 flex items-center justify-center px-4 transition-opacity duration-300 bg-black bg-opacity-50 z-50">
        <div className="bg-white rounded-lg shadow-2xl p-6 sm:p-8 w-full max-w-md transition-transform duration-300 transform scale-100">
          ...
          <div className="text-center mb-6 flex flex-col items-center">
            ...
            <h2 className="text-2xl text-gray-800 mt-4">{t("changepw.title")}</h2>
          </div>
          ...
          >
            {["Old Password", "New Password", "Confirm Password"].map((label, idx) => (
              <div key={idx}>
                <label className="block text-2xl font-medium text-gray-700 mb-1">{t(`changepw.${label.replace(" ", "").toLowerCase()}`)}</label>
                <input
                  type="password"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                  placeholder={t(`changepw.${label.replace(" ", "").toLowerCase()}`)}
                />
              </div>
            ))}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <button
                type="button"
                onClick={() => (verificationSent ? handleConfirmPassword() : handleSendVerification())}
